        ...
        for i in range(self.nl):
            ...
            if self.stride is not None:
                ...
                if self.training:
                    ...
                    x[i] = x[i] * anchor_grid + grid * stride
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5n.yaml', ch=3, nc=5, anchors=None, colors=2, tags=3, numpoints=5):  # model, input channels, number of classes
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], arg=(self.numpoints,self.colors,self.tags))  # model, savelist
...
def parse_model(d, ch, arg):  # model_dict, input_channels(3)
    LOGGER.info('\n%3s%18s%3s%10s  %-40s%-30s' % ('', 'from', 'n', 'params', 'module', 'arguments'))
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    no = na * (nc + 1 + arg[0])  # number of outputs = anchors * (classes + 5)
